...
function TableHeader<T extends IdEntity>(props: TableHeaderProps<T>) {
  const formApiRef = useRef<FormApi | undefined>(undefined);
  const [showCollapseColumn, setShowCollapseColumn] = useState<boolean>(false);
  ...
  const searchColumns = columns
    ...
    .map((col) => {
      return { ...col, span: tableContext.decorator.getDefaultSpan(col) || 6 };
    });
  ...
  return (
    ...
    >
      <div className="w-[80%]">
        ...
        <Collapsible isOpen={showCollapseColumn}>
      ...
      <Space className="ml-auto mr-2 ">
        <Button
          ...
          onClick={() => setShowCollapseColumn(!showCollapseColumn)}
        >
          更多
        </Button>
        <Button
